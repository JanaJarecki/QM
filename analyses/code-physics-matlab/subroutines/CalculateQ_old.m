function [Q_T,Qs] = CalculateQ(levelObject,path)
...
for n = lenT:-1:1
    chi(:,n) = propegateInTime(chi(:,n+1),xArray,timeStep,kinematicFactor,V(u(n,:)).',false,false); % this is only correct for linear Schr�dinger equation
end
% Fidelity
...
xi=zeros(lenX,lenT);
for n=1:lenT
    xi(:,n)=1/sqrt(F*(1-F))*(dx*chi(:,n)'*psi(:,n)*chi(:,n)-F*psi(:,n));
end
...
Qs=zeros(lenT,1);
for n=1:lenT
   Qs(n)=imag(xi(:,n)'*(T+diag(levelObject.potential(u(n,:))))*psi(:,n));
end
